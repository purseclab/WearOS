...
import soot.jimple.infoflow.plugin.wear.flowAnalyser.FlowMatch;
import soot.jimple.infoflow.plugin.wear.flowAnalyser.GlobalStatistics;
import soot.jimple.infoflow.plugin.wear.flowAnalyser.MatchingFlowAnalysis;
...
public class MainClass {
	...
	// Wear analysis extension
	private static final String OPTION_WEAR_FLOW_ANALYSIS = "wfa";
	private static final String OPTION_STRING_ANALYSIS = "wsa";
	private static final String OPTION_STRING_CONFIG = "wsc";
	private static final String OPTION_STATISTICS = "wss";
	...
	private void initializeCommandLineOptions() {
		...
		// Wear extension
		options.addOption(OPTION_WEAR_FLOW_ANALYSIS, "analysiswearflows", false,
				"Merge flows from mobile and wear app");
		options.addOption(OPTION_STRING_ANALYSIS, "stringanalysiswear", false,
				"Calculate string values for wear analysis");

		options.addOption(OPTION_STRING_CONFIG, "stringconfig", true, "File containing the config for String analysis");

		options.addOption(OPTION_STATISTICS, "statistics", true, "File containing the global output");

	}

	public static void main(String[] args) throws Exception {
		MainClass main = new MainClass();
		GlobalStatistics.auxTime = System.nanoTime();
		main.run(args);
	...
	private void run(String[] args) throws Exception {
		...
		try {
			...
			// POINT: Matching Analysis
			if (config.getWearConfiguration().getOnlyFlowAnalysis()) {

				String path = config.getAnalysisFileConfig().getTargetAPKFile();

				String[] files = path.split(";");
				if (files.length == 2) {

					String mobileDir = files[0];
					String wearDir = files[1];

					MatchingFlowAnalysis flowAnalysis = new MatchingFlowAnalysis();
					flowAnalysis.matchFlows(mobileDir, wearDir);
					logger.info("List of matched flow:");
					for (FlowMatch f : flowAnalysis.getFlowsMatched())
						logger.info(f.toString());

				} else {
					logger.error("Only one file found");
				}

				return; // we don't continue with the execution
	...
	private ITaintPropagationWrapper initializeTaintWrapper(CommandLine cmd) throws Exception {
		...
		case "multi":
			...
			// We need to group the definition files by their type /media/frost/hardWolf/PurSec/WearOS/wearflowDebug/Wearflow-Fixes/FlowDroid/soot-infoflow-summaries/summariesManual
			MultiMap<String, String> extensionToFile = new HashMultiMap<>(definitionFiles.length);
			for (String str : definitionFiles) {
				File f = new File(str);
				logger.info("DEFINITION FILE: "+str);
				if (f.isFile()) {
	...
	private void parseCommandLineOptions(CommandLine cmd, InfoflowAndroidConfiguration config) {
		...
		if (cmd.hasOption(OPTION_WEAR_FLOW_ANALYSIS)) {
			config.getWearConfiguration().setOnlyFlowAnalysis(true);
		}

		if (cmd.hasOption(OPTION_STRING_ANALYSIS)) {
			config.getWearConfiguration().setStringAnalysis(true);
		}

		{
			String stringConfig = cmd.getOptionValue(OPTION_STRING_CONFIG);
			if (stringConfig != null && !stringConfig.isEmpty())
				config.getWearConfiguration().setStringConfigFile(stringConfig);
		}

		{
			String stats = cmd.getOptionValue(OPTION_STATISTICS);
			if (stats != null && !stats.isEmpty())
				config.getWearConfiguration().setStatisticPath(stats);
		}
