...
public class TaintWrapperSet implements IReversibleTaintWrapper {
	...
	protected static final Logger logger = LoggerFactory.getLogger(TaintWrapperSet.class);
	@Override
	...
	public Set<Abstraction> getTaintsForMethod(Stmt stmt, Abstraction d1, Abstraction taintedPath) {
		...
		// logger.info("TAINTED PATH {} Stmt=> " + stmt.toString() + " taintedPath => " + taintedPath.toString(),Thread.currentThread().getId());
		for (ITaintPropagationWrapper w : this.wrappers) {
			Set<Abstraction> curAbsSet = w.getTaintsForMethod(stmt, d1, taintedPath);
			if (curAbsSet != null && !curAbsSet.isEmpty()){
				// logger.info("TAINTS FOR METHOD: {} "+ curAbsSet.toString(),Thread.currentThread().getId());
		...
		if (resList.isEmpty()){
			// logger.info("MISS {}",Thread.currentThread().getId());
			misses.incrementAndGet();
		}
		else{
			// logger.info("HIT {}",Thread.currentThread().getId());
