...
public abstract class BaseEntryPointCreator implements IEntryPointCreator {
	...
	protected Stmt buildMethodCall(SootMethod methodToCall, Body body, Local classLocal, LocalGenerator gen,
		...
		if (classLocal == null && !methodToCall.isStatic()) {
			// logger.warn("Cannot call method {}, because there is no local for base
			// object: {}", methodToCall,
	...
	protected Local generateClassConstructor(SootClass createdClass, Body body, Set<SootClass> constructionStack,
		...
		if (createdClass.isPhantom() || createdClass.isPhantomClass()) {
			// logger.warn("Cannot generate constructor for phantom class {}",
		...
		if (createdClass.isInterface() || createdClass.isAbstract()) {
			...
			// logger.warn("Could not find a suitable constructor for class {}",
			// createdClass.getName());
	...
	private Local generateSubstitutedClassConstructor(SootClass createdClass, Body body,
		...
		if (!substituteCallParams) {
			// logger.warn("Cannot create valid constructor for {}, because it is {} and
			// cannot substitute with subclass",
			// createdClass,
