...
public class ARSCFileParser extends AbstractResourceParser {
	...
	private void readResourceHeader(InputStream stream) throws IOException {
		...
		//// logger.debug(String.format("Package Groups (%d)",
		//// resourceHeader.packageCount));
		...
		while (offset < remainingData.length - 1) {
			...
			if (nextChunkHeader.type == RES_STRING_POOL_TYPE) {
				...
				// logger.debug(
				// String.format("\tPackage %s id=%d name=%s", packageCtr, packageTable.id,
				// packageTable.name));
				...
				// logger.debug(String.format("\t\tType %s (%d), configCount=%d, entryCount=%d",
				// resType.typeName,
				// resType.id - 1, resType.configurations.size(),
				// resType.configurations.size() > 0 ?
				// resType.configurations.get(0).resources.size()
				// : 0));
