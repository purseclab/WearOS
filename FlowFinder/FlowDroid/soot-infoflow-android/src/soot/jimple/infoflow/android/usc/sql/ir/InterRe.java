package soot.jimple.infoflow.android.usc.sql.ir;
...
import soot.jimple.infoflow.android.usc.sql.string.Encoder;
...
public class InterRe {
	...
	public InterRe(List<NodeInterface> nodes, Map<String, Boolean> regionDef, List<String> lineDef, String folderName,
			Map<NodeInterface, List<String>> targetVarNodeAndName, Map<String, Set<NodeInterface>> paraMap,
			Map<String, Set<String>> field, int targetCount, String methodName, JavaApp App,
			Map<String, List<Integer>> targetSignature) {
	...
	private void interpret(NodeInterface n) {
			...
			if (targetSignature != null && actualNode.containsInvokeExpr()) {
				...
				if (targetSignature.keySet().contains(signature)) {
					...
					for (ValueBox vb : actualNode.getUseBoxes()) {
						if (vb.getValue().getType().toString().equals("java.lang.String")
								&& !vb.getValue().toString().contains("<")) {
							...
							if (targetSignature.get(signature).contains(paraNum)) {
								...
								// label: the target signature is found in method name at line source line +
								// bytecode offset
		...
		if (actualNode != null && lineDef.contains(n.getOffset().toString())) {
			...
			if (actualNode.toString().contains("@parameter"))
				lineUseMap.put(n.getOffset().toString(),
						new ExternalPara(sootUse.get(0).getValue().toString() + ":" + methodName));
			// case : new StringBuilder
			// !!
			else if (actualNode.toString().contains("= new java.lang.StringBuilder")
					|| actualNode.toString().contains("= new java.lang.StringBuffer"))
				lineUseMap.put(n.getOffset().toString(), new ConstantString(""));
			// case : valueOf or toString
			else if (actualNode.toString()
					.contains("staticinvoke <java.lang.String: java.lang.String valueOf(java.lang.Object)>")
			...
			// case : init
			else if (actualNode.toString().contains("<java.lang.StringBuilder: void <init>()>")
					|| actualNode.toString().contains("<java.lang.StringBuffer: void <init>()>")
					|| actualNode.toString().contains("<java.lang.String: void <init>()>")) {
				lineUseMap.put(n.getOffset().toString(), new ConstantString(""));
			} else if (actualNode.toString().contains("<java.lang.StringBuilder: void <init>(java.lang.String)>")
					|| actualNode.toString().contains("<java.lang.StringBuilder: void <init>(java.lang.CharSequence)>")
					|| actualNode.toString().contains("<java.lang.StringBuffer: void <init>(java.lang.String)>")
					|| actualNode.toString().contains("<java.lang.String: void <init>(java.lang.StringBuffer)>")
					|| actualNode.toString().contains("<java.lang.String: void <init>(java.lang.String)>")
					|| actualNode.toString().contains("<java.lang.String: void <init>(java.lang.StringBuilder)>")) {
			...
			// string manipulation
				...
				for (ValueBox vb : sootUse) {
					...
					if (!use.contains("virtualinvoke") && !use.contains("staticinvoke")) {
						...
						if (use.equals("null"))
						...
						} else if ((use.contains("b") || use.contains("i")) || use.contains("c") || use.contains("z")
								|| use.contains("d") || use.contains("l") && !use.contains("\"")) {
							...
							if (actualNode.toString()
									.contains("<java.lang.StringBuffer: java.lang.StringBuffer append(boolean)")) {
								if (use.equals("0"))
				...
				// lineUseMap.put(n.getOffset().toString(), new ConstantString("newarray"));
					...
					&& !actualNode.toString().contains("staticinvoke")
					&& !actualNode.toString().contains("specialinvoke")
					&& !actualNode.toString().contains("virtualinvoke")
					&& !actualNode.toString().contains("interfaceinvoke")) {
				...
				if (sootUse.size() == 1) {
					...
					// case: r = null
						...
						if (v instanceof FieldRef) {
							lineUseMap.put(n.getOffset().toString(),
									new ExternalPara(((FieldRef) v).getField().getSignature()));
					...
					if (actualNode.toString().matches("(.r|r)([0-9]*)(.*) = (.r|r)([0-9]*)(.*)")) {
						...
						for (ValueBox vb : sootUse) {
							if (!(vb instanceof JimpleLocalBox || vb instanceof ImmediateBox)) {
								...
								if (regionDef.get(use) == null) {
									if (use.contains(
											"[")/* &&vb.getValue().getType().toString().equals("java.lang.String[]") */) {
						...
						for (ValueBox vb : sootUse) {
							...
							&& actualNode.toString().contains("(java.lang.String[])")) {
						for (ValueBox vb : sootUse) {
							if (vb instanceof ImmediateBox) {
								String use = vb.getValue().toString();
								if (regionDef.get(use))
			...
			// r = *invoke
			else if (actualNode.toString().contains("staticinvoke") || actualNode.toString().contains("specialinvoke")
					|| actualNode.toString().contains("virtualinvoke")
					|| actualNode.toString().contains("interfaceinvoke")) {
				...
				// replace the externalPara with internal or the extenalPara in the current
				// method
				...
				if (!temp.isEmpty())
					...
					// if the method return an String but we don't have the summary for that method,
					// return method name.
					if (actualNode.getUseBoxes().get(0).getValue().getType().toString().equals("java.lang.String")) {
						...
						lineUseMap.put(n.getOffset().toString(),
								new ExternalPara(
										"!!!" + methodName + "@" + this.methodName + "@" + n.getOffset().toString()
												+ "@" + actualNode.getDefBoxes().get(0).getValue().toString() + "!!!"));
					}
		...
		else if (actualNode != null && actualNode.toString().contains("return")
				&& !actualNode.toString().contains("goto") && !actualNode.getUseBoxes().isEmpty()) {
			...
			if (actualNode.getUseBoxes().get(0).getValue().getType().toString().equals("java.lang.String")) {
				...
				&& actualNode.toString()
						.contains("<LoggerLib.Logger: void reportString(java.lang.String,java.lang.String)>")
				&& !actualNode.toString().contains("goto")) {
		...
		// *invoke
				...
				&& (actualNode.toString().contains("staticinvoke") || actualNode.toString().contains("virtualinvoke")
						|| actualNode.toString().contains("specialinvoke")
						|| actualNode.toString().contains("specialinvoke"))
				&& !actualNode.toString().contains("<init>") && !actualNode.toString().contains("goto")) {
