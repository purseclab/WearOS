...
import soot.jimple.infoflow.android.usc.sql.ir.ConstantString;
import soot.jimple.infoflow.android.usc.sql.ir.Expression;
import soot.jimple.infoflow.android.usc.sql.ir.ExternalPara;
import soot.jimple.infoflow.android.usc.sql.ir.InternalVar;
import soot.jimple.infoflow.android.usc.sql.ir.T;
import soot.jimple.infoflow.android.usc.sql.ir.Variable;
...
public class JavaAndroid {
	...
	private Map<String, ReachingDefinition> rds = new HashMap<String, ReachingDefinition>();
	private Map<String, CFGInterface> cfgs = new HashMap<String, CFGInterface>();
	private StringCallGraph callGraph;
	private List<StringResult> stringResults = new ArrayList<StringResult>();

	public JavaAndroid(String rtjar, String appfolder, String classlist, String apk,
			Map<String, List<Integer>> targetSignature, int maxloop) {
	...
	public JavaAndroid(String rtjar, String appfolder, String classlist, Map<String, List<Integer>> targetSignature,
			int maxloop) {
		this.targetSignature = targetSignature;
		this.maxloop = maxloop;
		InterpretCheckerJava(rtjar, appfolder, appfolder + classlist, appfolder + "/MethodSummary/",
				appfolder + "/Output/");
	}

	public JavaAndroid(Map<String, List<Integer>> targetSignature, int maxloop, String outputPath, String packageName) {
		this.targetSignature = targetSignature;
	...
	private void InterpretCheckerAndroid(String outputPath) {
		...
		long t1, t2;
		// POINT prune the call graph and only include callers and callees of target
		// APIs
		...
		for (Entry<String, Map<String, Set<Variable>>> enout : targetMap.entrySet()) {
			...
			for (Entry<String, Set<Variable>> irSignature : enout.getValue().entrySet()) {
				...
				// statistic.add(en.getKey()+":"+getWidth(newIR)+" "+getHeight(newIR)+"
				// "+getLoopDepth(newIR)+" "+getLoopCount(newIR)+" "+getExternalCount(newIR));
				...
				// if(!emptyOrContainUnknown(value))
				{
					...
					SootMethod sm = parseMethodName(hotspot[0]);
					if (sm == null)
						continue;
					// sm,line,nth param,value
					StringResult tmp = new StringResult(sm, hotspot[1], hotspot[3], hotspot[4], hotspot[2]);
					stringResults.add(tmp);
					...
					for (Variable targetIR : newIR) {
						...
						for (String intpValue : targetIR.getInterpretedValue()) {
							...
							// add values also to a StringAnalysis result to handle in the instrumentation
							tmp.getValues().add(replace);
							possibleValues.add(replace);
						}
					}
					// System.out.println();
					// Method Signature@Bytecode Offset@Parameter Index
					analysisResult.put(hotspot[0] + "@" + hotspot[1] + "@" + hotspot[2] + "@" + hotspot[3],
							possibleValues);

					// try {
					// if (outputPath != null) {
					// PrintWriter bw = new PrintWriter(new FileWriter(outputPath + "result.txt",
					// true));
					// bw.println(result);
					//
					// bw.flush();
	...
	private SootMethod parseMethodName(String name) {
		String[] split = name.split(":");
		String classNameLong = split[0];
		String rest = split[1];

		// parse the name
		classNameLong = classNameLong.replaceAll("<", "");
		SootClass sclass = Scene.v().getSootClass(classNameLong);
		// parse sub-signature
		rest = rest.replaceFirst(" ", "");
		String subsignature = rest.replaceAll(">$", "");
		SootMethod sm = sclass.getMethodUnsafe(subsignature);
		return sm;
	}
	...
	private void InterpretCheckerJava(String arg0, String arg1, String arg2, String summaryFolder, String wfolder) {
		...
		if (arg1.contains("bookstore")) {
			...
			App = new JavaApp(arg0, arg1, arg2,
	...
	private boolean isFrameworkClass(SootMethod sm) {
		String tmpPackageName = sm.getDeclaringClass().getPackageName();
		if (!tmpPackageName.startsWith(this.packageName)) {
			return true;
		}
		// List<String> frameworkClasses = new ArrayList<String>(
		// Arrays.asList("java.", "org.joda", "android.", ".apache", "com.google",
		// "androidx.", "com.android",
		// "com.google", "com.sun", "dalvik", "javax.", "sun", "libcore.", "gov.nist"));
		// for (String st : frameworkClasses) {
		// if (tmpPackageName.startsWith(st))
		// return true;
		// }
	...
	public Set<String> identifyRelevant(Set<String> targetScanList) {
		...
		List<String> wearableCallbacks = new ArrayList<String>(Arrays.asList(ExtraTypes.WEAR_CALLBACKS));
		for (NewNode n : rto) {
			SootMethod contextMethod = n.getMethod();
			if (!isFrameworkClass(contextMethod)) {
					...
					for (Unit actualNode : n.getMethod().retrieveActiveBody().getUnits()) {
						...
						if (((Stmt) actualNode).containsInvokeExpr()) {
							...
							try {
								...
								System.out
										.println("Soot fails to get the method:" + ((Stmt) actualNode).getInvokeExpr());
							}
							if (sm == null)
								continue;
							...
							if (targetScanList.contains(sig) || targetMethod.contains(sig)) {
								...
								if (className.equals("java.lang.String")) {
									// String methods are only relevant if the context is a Wearable callback
									if (wearableCallbacks.contains(contextMethod.getName()))
										isRelevant = true;
								} else {
									isRelevant = true;
								}
		...
		// System.out.println("Target methods and callers and callees: "+
		// targetMethod.size());
		...
		// System.out.println("Total Target Method including constructor: "+
		// targetMethod.size());
		return targetMethod;
	}
