...
public class ProcessManifest implements Closeable {
	...
	// wearable listener definition
	protected HashMap<String, Set<String>> wearPaths = null;
	 ...
	 *             can occur due to a malformed manifest.
	 */
	// FIXME NA: here we read the manifest
	protected void handle(InputStream manifestIS) throws IOException, XmlPullParserException {
		...
		// parse intent filter for WearListenerServices
		this.wearPaths = getPathFromServices(this.services);
	}

	private HashMap<String, Set<String>> getPathFromServices(List<AXmlNode> services) {
		List<String> wearActions = new ArrayList<String>(Arrays.asList(ExtraTypes.wearActionsListener));
		HashMap<String, Set<String>> table = new HashMap<String, Set<String>>();

		for (AXmlNode service : services) {
			String componentName = service.getAttribute("name").getValue().toString();
			String[] splitName = componentName.split("[.]");
			componentName = splitName[splitName.length - 1];
			List<AXmlNode> intentFilters = service.getChildrenWithTag("intent-filter");

			for (AXmlNode intentFilter : intentFilters) {
				List<AXmlNode> action = intentFilter.getChildrenWithTag("action");
				List<AXmlNode> data = intentFilter.getChildrenWithTag("data");
				if (action != null) {
					for (int j = 0; j < action.size(); j++) {
						String actionName = action.get(j).getAttribute("name").getValue().toString();
						if (wearActions.contains(actionName)) {
							if (data != null) {
								for (int k = 0; k < data.size(); k++) {
									if (data.get(k).getAttribute("pathPrefix") != null) {
										String pathPrefix = data.get(k).getAttribute("pathPrefix").getValue()
												.toString();
										Set<String> value = new HashSet<String>();
										value.add("pathPrefix");
										value.add(pathPrefix);
										table.put(componentName, value);
										break;
									} else if (data.get(k).getAttribute("path") != null) {
										String path = data.get(k).getAttribute("path").getValue().toString();
										Set<String> value = new HashSet<String>();
										value.add("path");
										value.add(path);
										table.put(componentName, value);
										break;
									} else if (data.get(k).getAttribute("pathPattern") != null) {
										String pathPattern = data.get(k).getAttribute("pathPattern").getValue()
												.toString();
										Set<String> value = new HashSet<String>();
										value.add("pathPattern");
										value.add(pathPattern);
										table.put(componentName, value);
										break;
									}
	...
	public HashMap<String, Set<String>> getWearPaths() {
		return wearPaths;
	}

	public void setWearPaths(HashMap<String, Set<String>> wearPaths) {
		this.wearPaths = wearPaths;
	}
