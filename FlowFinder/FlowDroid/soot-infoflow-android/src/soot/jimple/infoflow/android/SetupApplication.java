...
import soot.jimple.infoflow.android.plugin.wear.analysis.StringAnalysis;
import soot.jimple.infoflow.android.plugin.wear.transformers.DeobfuscatorSceneTransformer;
import soot.jimple.infoflow.android.plugin.wear.transformers.WearSceneTransformer;
import soot.jimple.infoflow.android.plugin.wear.transformers.SensorEventSceneTransformer;
import soot.jimple.infoflow.android.plugin.wear.transformers.FrostDeobfuscator;
...
import soot.jimple.infoflow.plugin.wear.flowAnalyser.FlowsExporter;
...
public class SetupApplication implements ITaintWrapperDataFlowAnalysis {
	...
	protected List<SootClass> toInstrument;

	protected static FlowsExporter exporter;
	...
	/**
	 * POINT Triggers the callgraph construction in Soot
	 */
	...
	private void calculateCallbackMethods(LayoutFileParser lfp, SootClass component) throws IOException {
		...
		// POINT debug and check how to handle callbacks from GSM
		...
		logger.info("ENTRY POINT CLASSES");
		logger.info(entryPointClasses.toString());
		...
		try {
			...
			logger.error("Could note calculate callback methods", ex);
			throw ex;
		} finally {
	...
	/**
	 * POINT Initializes soot for running the soot-based phases
	 */
	...
	public InfoflowResults runInfoflow(ISourceSinkDefinitionProvider sourcesAndSinks) {
		...
		// Identify heavy obfuscation
		// deFroster();

		// POINT deobfuscation transformer
		// toInstrument = deobfuscateApp();

		// Frost Sensor Instrumentation
		sensorEventTransform();
...

		{
			...
			// ************************** POINT: Export the flows ************************\\

			String apkPath = config.getAnalysisFileConfig().getTargetAPKFile();
			String resultpath = apkPath.replace(".apk", ".json");
			exporter = FlowsExporter.getInstance();
			exporter.exportFlows(resultpath, resultAggregator.getAggregatedResults());
	...
	protected void processEntryPoint(ISourceSinkDefinitionProvider sourcesAndSinks,
		...
		// POINT: wear instrumentation, Do we need to call generateCallGraph?
		/*********************************************************************************************************/

		// addWearIntrumentation(toInstrument);
	...
	/**
	 * Start the instrumentation of the apk. Add a new SceneTransformer to the pack
	 * and run the packs
	 */
	private void addWearIntrumentation(List<SootClass> toInstrument) {
		...
		if (config.getWearConfiguration().getStringAnalysis()
				&& (config.getWearConfiguration().getStringConfigFile() != null)) {

			String pathConfig = config.getWearConfiguration().getStringConfigFile();
			StringAnalysis.runStringAnalysis(pathConfig, packageName,
					config.getAnalysisFileConfig().getAndroidPlatformDir(),
					config.getAnalysisFileConfig().getTargetAPKFile());
			result = StringAnalysis.getStringResults();

		}

		PackManager.v().getPack("wjtp")
				.add(new Transform("wjtp.MyInstrumentation", new WearSceneTransformer(manifest.getWearPaths(), result,
						packageName, toInstrument, config.getAnalysisFileConfig().getTargetAPKFile())));
		PackManager.v().getPack("wjtp").apply();
		PackManager.v().getPack("wjtp").remove("wjtp.MyInstrumentation");
	...
	private List<SootClass> deobfuscateApp() {
		...
		PackManager.v().getPack("wjtp").apply();
		PackManager.v().getPack("wjtp").remove("wjtp.MyDeobfuscation");
		return deobfuscator.getComponentsToInstrument();
	}

	private void sensorEventTransform() {
		logger.info("=++++++ sensorEventTransform ++++++");
		SensorEventSceneTransformer sensorTransformer = new SensorEventSceneTransformer();
		PackManager.v().getPack("wjtp").add(new Transform("wjtp.FrostTransform", sensorTransformer));//
		PackManager.v().getPack("wjtp").apply();
		PackManager.v().getPack("wjtp").remove("wjtp.FrostTransform");
	}

	private void deFroster() {
		logger.info("=++++++ deFrosting ++++++");
		FrostDeobfuscator frostDeobfuscator = new FrostDeobfuscator();
		PackManager.v().getPack("wjtp").add(new Transform("wjtp.FrostDeobfuscation", frostDeobfuscator));//
		PackManager.v().getPack("wjtp").apply();
		PackManager.v().getPack("wjtp").remove("wjtp.FrostDeobfuscation");
	}
