...
import soot.jimple.infoflow.android.data.parsers.FrostWearSinkParser;
...
public class PermissionMethodParser implements ISourceSinkDefinitionProvider {
	...
	private boolean addedWearSinks = false;
	...
	private void addWearSinks() {
		logger.info("Adding Wear Sinks");
		// Identify Wearable Class
		boolean wearableObfuscated = true;
		SootClass wearableClass = null;
		if (Scene.v().containsClass("com.google.android.gms.wearable.Wearable")){
			...
			wearableObfuscated = false;
			logger.info("Wearable Class Found: Not obfuscated");
		}
        //  TODO: Add check for application class

		if (!wearableObfuscated) {
			for (SootClass sClass : Scene.v().getApplicationClasses()) {
				if (wearableClass != null) {
					logger.info("Wearable Class Found: Obfuscated " + wearableClass.getPackageName());
					break;
				}
				try {
		...
		// FrostWearSinkParser wearSinkParser = new FrostWearSinkParser();
		// List<String> targetSinkMethods = new ArrayList<String>();
		// targetSinkMethods.addAll(wearSinkParser.getDataMapSinks(wearableClass.getPackageName()));
		// for (String targetSinkMethod : targetSinkMethods) {
		// 	logger.info(targetSinkMethod.toString()+" -> _SINK_");
		// 	this.data.add(targetSinkMethod.toString()+" -> _SINK_");
		// }

		// Array of target package names
		String[] targetSinkPackages = {
		...
		List targetSinkList = Arrays.asList(targetSinkPackages);

		// List all functions in Wearable Class
		if (wearableClass != null && wearableObfuscated) {
			String wearablePackageName = wearableClass.getPackageName();
			for (SootClass sClass : Scene.v().getApplicationClasses()) {
				if(sClass.getPackageName().equals(wearablePackageName)) {
					logger.info("WearableClass " + sClass.getName());
					for (SootMethod sm : sClass.getMethods()) {
						logger.info("<" + sClass.getName() + ": " + sm.getReturnType() + " " + sm.getName() + "(" + sm.getParameterTypes().toString().substring(1, sm.getParameterTypes().toString().length()-1) + ")> -> _SINK_");
						this.data.add("<" + sClass.getName() + ": " + sm.getReturnType() + " " + sm.getName() + "(" + sm.getParameterTypes().toString().substring(1, sm.getParameterTypes().toString().length()-1) + ")> -> _SINK_");
					}
				}
			}
		} else if (!wearableObfuscated) {
			logger.info("Adding non-obfuscated Wearable Sinks");
			for (SootClass sClass : Scene.v().getApplicationClasses()) {
				if(targetSinkList.contains(sClass.getName())) {
					logger.info("WearableClass " + sClass.getName());
					for (SootMethod sm : sClass.getMethods()) {
						logger.info("<" + sClass.getName() + ": " + sm.getReturnType() + " " + sm.getName() + "(" + sm.getParameterTypes().toString().substring(1, sm.getParameterTypes().toString().length()-1) + ")> -> _SINK_");
						this.data.add("<" + sClass.getName() + ": " + sm.getReturnType() + " " + sm.getName() + "(" + sm.getParameterTypes().toString().substring(1, sm.getParameterTypes().toString().length()-1) + ")> -> _SINK_");
